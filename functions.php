...
if ( ! function_exists( 'global_reporting_program_setup' ) ) :
	...
	function global_reporting_program_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'global_reporting_program_custom_background_args',
				array(
 ...
 */
function global_reporting_program_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'global-reporting-program' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'global-reporting-program' ),
...
add_action( 'widgets_init', 'global_reporting_program_widgets_init' );
...
function global_reporting_program_scripts() {
  wp_enqueue_style('font', 'https://fonts.googleapis.com/css2?family=Gothic+A1:wght@500;800&display=swap');
	...
	wp_style_add_data( 'global-reporting-program-style', 'rtl', 'replace' );
